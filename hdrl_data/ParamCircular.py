    ...
    def execute(self, obj):
        ...
        def sichu(self):
            ...
            for i in range(2):
                ...
                c00=c00.cut(c01)
            c01= Part.makeCylinder(17.0,h,Base.Vector(0,0-R,0),Base.Vector(0,0,1),360)
            c00=c00.fuse(c01)
        ...
        def Corner2_arc(self):#円弧 
            ...
            for i in range(n):
                if i==0:
                    h1=h
                    edge2 = Part.makeCircle(21.35, Base.Vector(0,-R,h1), Base.Vector(1,0,0), 0, 360)
                    if type=='03_Circular_arc':
                        edge2.Placement=App.Placement(App.Vector(0,0,0),App.Rotation(App.Vector(0,0,1),0))
                elif i==1:
                    if spec_siyo=='General' :
                        h1=h/2
                    else:
                        h1=100    
                    edge2 = Part.makeCircle(17.0, Base.Vector(0,-R,h1), Base.Vector(1,0,0), 0, 360)
                elif i==2:
                    if spec_siyo=='Sewerage Works Agency':
                        return
                    h1=20
        ...
        if spec_siyo=='General':#一般
            ...
            for i in range(n1+1):
                if i==0:
                    c02= Part.makeCylinder(11.0,h-100.0,Base.Vector(0,-R,100),Base.Vector(0,0,1),360)
                else:
                    c02= Part.makeCylinder(11.0,h-100.0,Base.Vector(0,-R,100),Base.Vector(0,0,1),360)
            ...
            for i in range(n+1):
                if i==0:
                    c1=Part.makeBox(60,60,h-50,Base.Vector(-30,-R-30,0),Base.Vector(0,0,1)) 
                    c1.Placement=App.Placement(App.Vector(0,0,0),App.Rotation(App.Vector(0,0,1),4*k1)) 
                else:
                    c2=Part.makeBox(60,60,h-50,Base.Vector(-30,-R-30,0),Base.Vector(0,0,1)) 
