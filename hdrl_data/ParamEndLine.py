...
class EndLine:#01 02
    ...
    def execute(self, obj):
        ...
        def sichu(self):
            ...
            if spec_siyo=='1':
                for i in range(6):
                    c01= Part.makeCylinder(11.0,h-100,Base.Vector(0,0,0),Base.Vector(0,0,1),360)
                    c00=c00.fuse(c01)
            ...
            for i in range(3):
                if i==0:
                    ...
                    if type=='01_Corner with end':
                        edge2 = Part.makeCircle(21.35, Base.Vector(50,0,h1), Base.Vector(1,0,0), 0, 360)
                    else:
                        #pass
                        edge2 = Part.makeCircle(21.35, Base.Vector(0,0,h1), Base.Vector(1,0,0), 0, 360)
                        #pass
                elif i==1:
                    if spec_siyo=='General':
                        ...
                        h1=100    
                    edge2 = Part.makeCircle(17.0, Base.Vector(50,0,h1), Base.Vector(1,0,0), 0, 360)
                elif i==2:
                    ...
                    edge2=Part.makePolygon([p4,p5,p6,p7,p4])
                if i==1 or i==2:
                  p1=(0,0,h1)
                else:
                    if type=='01_Corner with end':
        ...
        def Corner20a(self):
            ...
            for i in range(2):
                if i==0:
                    ...
                    pface=Part.makePlane(H,B,Base.Vector(0,0,h1-50), Base.Vector(1,0,0))
                    c00=pface.extrude(Base.Vector(l1+x,0,0))
                    pface.Placement=App.Placement(App.Vector(l1-x,0,0),App.Rotation(App.Vector(0,0,1),0))
                    c01=pface.extrude(Base.Vector(l2+x,0,0))
                    ...
                    if k>0:
                        ...
                        x=B/2*math.tan(s/2)-B*math.tan(s/2)
                    pface=Part.makePlane(H,B,Base.Vector(0,-12.5,h1), Base.Vector(1,0,0))
                    c02=pface.extrude(Base.Vector(l1+x,0,0))
                    pface.Placement=App.Placement(App.Vector(l1-x,0,0),App.Rotation(App.Vector(0,0,1),0))
                    c03=pface.extrude(Base.Vector(l2+x,0,0))
        ...
        if spec_siyo=='General':#一般手すり
            if type=='01_Corner with end':
                ...
                if (l1-p/2-n1*p)>=p:
                    ...
                    #sichu(self)    
                    #c1=c00
                    #c1.Placement=App.Placement(App.Vector(((l1-500)*p/1000),0,0),App.Rotation(App.Vector(0,0,1),180))    
                    if l1<=1500:
                        pass
                    else:
            ...
            for i in range(n+1):
                #支柱
                c2=Part.makeBox(60,60,h-50,Base.Vector(0,-65,0),Base.Vector(0,0,1))
                if i==0:
            ...
            n=int((l2-420)/1950)
            for i in range(n+1):
                c2=Part.makeBox(60,60,h-50,Base.Vector(l1-30,-65,0),Base.Vector(0,0,1))
                if i==0:
