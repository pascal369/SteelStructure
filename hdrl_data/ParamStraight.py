...
class StraightLine:#00
    ...
    def execute(self, obj):
        ...
        def h_rail(self):#一般_手すり
            ...
            #笠木
            c00= Part.makeCylinder(21.35,L0,Base.Vector(0,0,h),Base.Vector(1,0,0),360)
            #下弦材
            c01= Part.makeCylinder(17.0,L0,Base.Vector(0,0,h/2),Base.Vector(1,0,0),360)
            c00=c00.fuse(c01)
            if Reverse==True:
                cx=int(-1)
        ...
        def h_rail2(self):#事業団_手すり
            ...
            c00= Part.makeCylinder(21.35,L0,Base.Vector(0,0,h),Base.Vector(1,0,0),360)
            #下弦材
            c01= Part.makeCylinder(17.0,L0,Base.Vector(0,0,100),Base.Vector(1,0,0),360)
            c00=c00.fuse(c01)
        def SWA_Al(self):#事業団アルミ手すり
            global c00
            c00=Part.makeBox(50,70,l1,Base.Vector(0,0,h-50),Base.Vector(1,0,0))
            c2=Part.makeBox(33,45,l1,Base.Vector(0,-12.5,100),Base.Vector(1,0,0))
            c00=c00.fuse(c2)
            l00=1950
            n=int(l1/l00)
            x2=(l1-l00*n)/2
            ...
            if n>=1:
                for i in range(n+1):
                    if i==0:
                        c2=Part.makeBox(60,60,h-50.0,Base.Vector(x2-30,-35-30,0),Base.Vector(0,0,1))
                        c00=c00.fuse(c2)
                    else :
                        c2=Part.makeBox(60,60,h-50.0,Base.Vector(x2-30,-35-30,0),Base.Vector(0,0,1))
                ...
                c2=Part.makeBox(60,60,h-50,Base.Vector(l1/2-30,-35-30,0),Base.Vector(0,0,1))
                c00=c00.fuse(c2)
            #ラチス    
            for j in range(n+1):
                x2=(l1-1950*n)/2
                ...
                if n==0:
                    for i in range(n1):
                        c2=Part.makeBox(20,20,h-183.0,Base.Vector(x2+150-10,-45,133),Base.Vector(0,0,1))
                ...
                if j==0:
                    if n==0:
                        ...
                        n00=13
                    for i in range(n00):
                        c2=Part.makeBox(20,20,h-183,Base.Vector(x2+150-10,-45,133),Base.Vector(0,0,1))
                        c2.Placement=App.Placement(App.Vector(i*150,0,0),App.Rotation(App.Vector(1,0,0),0))
                        c00=c00.fuse(c2)
                    for i in range(n1):
                        c2=Part.makeBox(20,20,h-183,Base.Vector(x2-150-10,-45,133),Base.Vector(0,0,1))
                    ...
                    for i in range(n1):
                        c2= Part.makeBox(20,20,h-183,Base.Vector((n)*l00+x00-10,-45,133),Base.Vector(0,0,1))  
                        c2.Placement=App.Placement(App.Vector(i*150,0,0),App.Rotation(App.Vector(1,0,0),0))
                        c00=c00.fuse(c2)
                        
                else:
                    for i in range(13):
                        c2= Part.makeBox(20,20,h-183,Base.Vector((j)*l00+x00-150-10,-45,133),Base.Vector(0,0,1))  
                        c2.Placement=App.Placement(App.Vector(i*150,0,0),App.Rotation(App.Vector(1,0,0),0))
                        c00=c00.fuse(c2)  
        ...
        if spec_siyo=='General':
            ...
            for j in range(n+2):
                ...
                if j==0:
                    for i in range(n1+1): 
                        c2= Part.makeCylinder(11.0,h-100,Base.Vector(x2,0,0),Base.Vector(0,0,1),360)  
                    ...
                    for i in range(n1+1):
                        ...
                        c2= Part.makeCylinder(11.0,h-100,Base.Vector((j-1)*l00+x2,0,0),Base.Vector(0,0,1),360)  
                        c2.Placement=App.Placement(App.Vector(i*150,0,100),App.Rotation(App.Vector(1,0,0),0))
                        c1=c1.fuse(c2)
                else:
                    for i in range(13): 
